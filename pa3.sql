...
WHERE r.user_id = (SELECT u.id FROM user u WHERE u.user_password = 'ForestWonder');
...
WHERE t.id IN (SELECT tr.trail_id FROM trail_rating tr 
    INNER JOIN difficulty d ON tr.difficulty_id = d.id WHERE d.name = 'Hard');
...
WHERE t.id = (
    SELECT tr.trail_id FROM user u
...
UPDATE user u 
SET u.type = 'noob'
WHERE EXISTS (SELECT tr.user_id FROM trail_rating tr WHERE tr.user_id = u.id AND tr.score < 0.5);
